...
export const AppointmentForm = ({ mode = "create", initialStart = "", initialEvent, onClose, setIsPosting, doctorBySpecialty, onEventSaved, }: AppointmentFormProps) => {
  ...
  const onErrorMessage = (value: string): void => {
    toast.message(value, { duration: 6000 });
  }
  const { createMutation, updateMutation, isPosting } = useAppointmentMutation(onSuccessCallback, setError, onErrorMessage);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex justify-end gap-3">
        <Button type="button" variant="outline" onClick={() => onClose(false)} disabled={isPosting}>
          Cancelar
        </Button>
